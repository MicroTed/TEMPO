    ...
    if (any(l_qr)) then
      ...
      if (tempo_cfgs%semi_sedi_flag) then
        ...
        do n = 1, substeps_sedi
          ...
          call rain_check_and_update(rho, l_qr, xrx, xnx, rr, nr, qrten, nrten, ilamr, mvd_r, dt, odt) 
          call rain_fallspeed(rhof=rhof, l_qr=l_qr, rr=rr, ilamr=ilamr, dz1d=dz1d, &
            vt=vtrr, vtn=vtnr, vtz=vtzr, dt=dt, ssflg=tempo_cfgs%rssflg)
        enddo 
      else
...
!             vt=vtrr, vtn=vtnr, vtz=vtzr, ssflg=tempo_cfgs%rssflg)
    ...
    if (any(l_qg)) then
      call graupel_fallspeed(rhof=rhof, rho=rho, visco=visco, &
        ...
        dt=dt, igrfallopt=tempo_cfgs%igrfallopt, ssflg=tempo_cfgs%hssflg)
      ...
      if (tempo_cfgs%semi_sedi_flag) then 
        ...
        do n = 1, substeps_sedi
          ...
          call graupel_fallspeed(rhof=rhof, rho=rho, visco=visco, &
            l_qg=l_qg, rg=rg, rb=rb, qb1d=qb1d, zg=zg, idx=idx_bg, ilamg=ilamg, dz1d=dz1d, &
            vt=vtrg, vtn=vtng, vtz=vtzg, dt=dt, igrfallopt=tempo_cfgs%igrfallopt, ssflg=tempo_cfgs%hssflg)
        enddo 
      else
...
!             vt=vtrg, vtn=vtng, vtz=vtzg, igrfallopt=tempo_cfgs%igrfallopt, ssflg=tempo_cfgs%hssflg)
 ...
 subroutine graupel_check_and_update(rho, l_qg, qg1d, ng1d, qb1d, rg, ng, rb, &
  ...
  subroutine rain_fallspeed(rhof, l_qr, rr, ilamr, dz1d, vt, vtn, vtz, substeps_sedi, ktop_sedi, &
    ...
    use module_mp_tempo_params, only : crg, av_r, org2, org3, org4, fv_r, cre
    ...
    do k = nz, 1, -1
      if (rr(k) > r1) then
        ...
        vtz(k) = 0
        if ( ssflg >=2 ) then 
          vtz(k) = rhof(k)*av_r*crg(14)*org4 * lamr**cre(15) *(lamr+fv_r)**(-cre(14))
        ! check this!!!
          vtn(k) = rhof(k)*av_r*crg(5)*org2 * lamr**cre(2)*(lamr+fv_r)**(-cre(5))
        else
          vtz(k) = 0.
  ...
  subroutine graupel_fallspeed(rhof, rho, visco, l_qg, rg, rb, zg, qb1d, idx, ilamg, &
      dz1d, vt, vtn, vtz, substeps_sedi, ktop_sedi, dt, igrfallopt, ssflg)
    !! calculates mass and number weighted fall speeds for graupel
    !! and optionally the substepping required and the top k-level of sedimentation
    use module_mp_tempo_params, only : nrhg, rho_g, av_g_old, bv_g_old, &
      cgg, t0, mu_g, ogg2, ogg3, ogg4, a_coeff, b_coeff, meters3_to_liters, &
      rho_not, earth_gravity
